...
class StorageManager {
    ...
    saveLastSettings(settings) {
        try {
            localStorage.setItem(CONFIG.STORAGE_KEYS.LAST_SETTINGS, JSON.stringify(settings));
            return true;
        } catch (e) {
    ...
    cacheTasks(tag, tasks) {
        try {
            ...
            localStorage.setItem(CONFIG.STORAGE_KEYS.TASKS_CACHE, JSON.stringify(cache));
            return true;
        } catch (e) {
    ...
    getCachedTasks(tag) {
        ...
        console.log(`Кэш устарел (версия ${cached.version}, текущая ${CONFIG.DATA_VERSION})`);
        return null;
        }
